...
std::string sha256_file(const std::filesystem::path& path)
{
    ...
    if (!input) {
        throw std::runtime_error("could not open file for SHA-256 verification: "
                                 + path.string());
    }
    ...
    while (input) {
        ...
        if (bytes_read > 0) {
            hasher.update(std::as_bytes(
                std::span(buffer.data(), static_cast<std::size_t>(bytes_read))));
        }
    }

    if (!input.eof()) {
        throw std::runtime_error("failed while reading file for SHA-256 verification: "
                                 + path.string());
    }
    return hasher.finish();
}
